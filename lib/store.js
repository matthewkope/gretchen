...
function writeBoard(columns, sprint) {
  ...
  // board stays a time-boxed sprint that round-trips through Obsidian too)
...
// ── Sprint metadata (kanban.md frontmatter) ────────────────────────────────
// The board is a sprint: a number, a goal, and a start/end date stamped in the
// YAML frontmatter. Defaults to a fresh 2-week sprint starting today.
...
// parse just the sprint fields out of the frontmatter, no side effects on columns
function readSprint() {
...
// Start the next sprint: archive the Done column's cards, roll the rest forward,
// bump the number, and stamp fresh dates (same length as the sprint just ended).
