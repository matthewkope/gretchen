...
// local YYYY-MM-DD for a Toggl ISO start time (so it buckets by the same
// calendar day the local time.csv uses)
function localDate(iso) {
  const d = new Date(iso);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

// completed time entries in [startDate, endDate) (YYYY-MM-DD), normalised to
// { project, secs, date }. Entries with no Toggl project become "Untitled".
export async function fetchEntries(startDate, endDate) {
  const { projects } = await context();
  const byId = Object.fromEntries(projects.map((p) => [p.id, p.name]));
  const entries = (await api('GET', `/me/time_entries?start_date=${startDate}&end_date=${endDate}`)) || [];
  return entries
    .filter((e) => e.duration > 0) // skip the currently-running entry (duration < 0)
    .map((e) => ({
      project: (e.project_id && byId[e.project_id]) || 'Untitled',
      secs: e.duration,
