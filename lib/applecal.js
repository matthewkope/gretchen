...
const CONNECTED_FILE = path.join(os.homedir(), '.gretchen', 'applecal.json');

// Whether the user has connected Apple Calendar before. Persisted so events
// come back after a restart without re-connecting; the OS keeps the permission
// grant, so reading events again never re-prompts.
export function loadConnected() {
  try { return JSON.parse(fs.readFileSync(CONNECTED_FILE, 'utf8')).connected === true; } catch { return false; }
}

export function setConnected(on) {
  fs.mkdirSync(path.dirname(CONNECTED_FILE), { recursive: true });
  fs.writeFileSync(CONNECTED_FILE, `${JSON.stringify({ connected: !!on }, null, 2)}\n`);
}
