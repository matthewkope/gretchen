...
// every locally-logged time.csv entry, normalised to { project, secs, date }.
// Blank project → "Untitled" (matches Toggl's label for project-less entries).
export function localEntries() {
  const out = [];
  if (!fs.existsSync(CSV_FILE)) return out;
  const lines = fs.readFileSync(CSV_FILE, 'utf8').split('\n').slice(1).filter(Boolean);
  for (const line of lines) {
...
// [startDate, endDate) of the Mon–Sun week (YYYY-MM-DD), for querying Toggl.
...
// aggregate flat { project, secs, date } entries into the Mon–Sun week view
// (per-day bars + per-project totals) for the home "This week" widget.
export function aggregateWeek(entries, weekOffset = 0) {
  const now = new Date();
  ...
  for (const e of entries) {
    ...
    if (!day) continue;
    day.secs += e.secs;
    const proj = (e.project || '').trim() || 'Untitled';
    projects[proj] = (projects[proj] || 0) + e.secs;
    total += e.secs;
  }
