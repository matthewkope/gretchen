...
// city name → up to `count` matches [{ name, lat, lon, tz }] via the free
// Open-Meteo geocoder (no API key). Powers the location autocomplete.
export async function geocodeSearch(query, count = 6) {
  const q = (query || '').trim();
  if (!q) return [];
...
// city name → best single match { name, lat, lon, tz } (or null)
export async function geocode(query) {
  return (await geocodeSearch(query, 1))[0] || null;
}
